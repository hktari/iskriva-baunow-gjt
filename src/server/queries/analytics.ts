...
export const getGeneralAnalytics = cache(
...
export const getOrganizationAnalytics = cache(
  async (organizationId: string): Promise<OrganizationAnalyticsData> => {
    const whereClause = {
...
export const getOrganizationList = cache(async (): Promise<OrganizationOption[]> => {
  const organizations = await db.configurableField.findMany({
    where: {
      category: 'ORGANIZATION',
    },
    orderBy: {
...
function calculateProjectStatus(projects: any[]): ProjectStatusData[] {
...
function calculateEnvironmentalImpact(kpis: any[]): EnvironmentalImpactData[] {
